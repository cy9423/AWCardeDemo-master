...
- (void)creatMainButtonView{
    ...
    [btnView setChooseClick:^(NSInteger tag) {
        ...
        switch (tag) {
            case 0:
            {
                [weakSelf.collectionView setContentOffset:CGPointMake(0, weakSelf.collectionView.contentOffset.y) animated:true];
            }
                break;
            case 1:
            {
                [weakSelf.collectionView setContentOffset:CGPointMake((Item_Width)+20, weakSelf.collectionView.contentOffset.y) animated:true];
            }
                break;
            case 2:
            {
                [weakSelf.collectionView setContentOffset:CGPointMake((Item_Width)*2+20*2, weakSelf.collectionView.contentOffset.y) animated:true];
            }
                break;
            case 3:
            {
                [weakSelf.collectionView setContentOffset:CGPointMake((Item_Width)*3+60, weakSelf.collectionView.contentOffset.y) animated:true];
            }
                break;
            default:
                break;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    UIButton *sender = (UIButton *)[btnView viewWithTag:1000 + self.currentIndexPath.row];
    sender.selected = YES;
